...
load("//tools/cpp:windows_cc_configure.bzl", "configure_windows_toolchain")
load("//tools/cpp:unix_cc_configure.bzl", "configure_unix_toolchain")
load(
...
def cc_autoconf_toolchains_impl(repository_ctx):
    ...
    if should_detect_cpp_toolchain:
        paths = resolve_labels(repository_ctx, [
            "//tools/cpp:BUILD.toolchains.tpl",
        ])
        repository_ctx.template(
            "BUILD",
            paths["//tools/cpp:BUILD.toolchains.tpl"],
...
def cc_autoconf_impl(repository_ctx, overriden_tools = dict()):
    ...
    if "BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN" in env and env["BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN"] == "1":
        paths = resolve_labels(repository_ctx, [
            ...
            "//tools/cpp:empty_cc_toolchain_config.bzl",
        ])
        repository_ctx.symlink(paths["//tools/cpp:empty_cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.template("BUILD", paths["//tools/cpp:BUILD.empty.tpl"], {
        ...
        repository_ctx.symlink(paths["//tools/cpp:bsd_cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.symlink(paths["//tools/cpp:BUILD.static.bsd"], "BUILD")
